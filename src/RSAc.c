...
//        beta1=rand()*1.0/RAND_MAX*(up[2]-low[2])+low[2];
        for(j=1;j<=dataCol-2;j++) beta[j]=rand()*1.0/RAND_MAX*(up[1]-low[1])+low[1];
        logphi=rand()*1.0/RAND_MAX*(up[3]-low[3])+low[3];
        logsigmasq=rand()*1.0/RAND_MAX*(up[4]-low[4])+low[4]; 
        logtausq=rand()*1.0/RAND_MAX*(up[5]-low[5])+low[5];
